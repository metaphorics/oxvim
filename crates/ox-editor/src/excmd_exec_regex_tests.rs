...
fn setup(text: &str) -> (TestEditorAccess, BufHandle, WinHandle, ExExecutor) {
    ...
    (TestEditorAccess::new(editor), buffer, window, executor)
}
...
fn global_default_range_honors_current_line() {
    ...
    let (editor, _buffer, window, mut executor) = setup("foo\nbar\nbaz");
    editor
        .editor_mut()
        .set_window_cursor(window, position(2, 0))
        .unwrap();
    executor.execute_line(&editor, r"g/bar/d").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn global_whole_buffer_range_appends_to_all_matches() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("foo\nbar\nfoo");
    executor.execute_line(&editor, r"%g/foo/s/$/ END/").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn global_explicit_line_range_limits_matches() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("one\nbar1\nbar2\nfoo");
    executor.execute_line(&editor, r"2,3g/bar/s/$/ X/").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn vglobal_inverts_selection() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("foo\nbar\nbaz");
    executor.execute_line(&editor, r"%v/foo/s/$/ X/").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn global_missing_pattern_reports_e148() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("foo\nbar");
    let error = executor.execute_line(&editor, "g").unwrap_err();
...
fn global_invalid_regex_reports_e54() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("foo\nbar");
    let error = executor.execute_line(&editor, r"g/[/d").unwrap_err();
...
fn substitute_default_range_only_current_line() {
    ...
    let (editor, _buffer, window, mut executor) = setup("one\nfoo\nthree");
    editor
        .editor_mut()
        .set_window_cursor(window, position(2, 0))
        .unwrap();
    executor.execute_line(&editor, r"s/foo/bar/").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
    assert_eq!(text.line(2).unwrap(), b"bar");
...
fn substitute_first_match_only_without_g() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("foo foo");
    executor.execute_line(&editor, r"s/foo/bar/").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn substitute_global_flag_replaces_every_match() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("foo foo");
    executor.execute_line(&editor, r"s/foo/bar/g").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn substitute_no_match_errors_e486() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("foo");
    let error = executor.execute_line(&editor, r"s/missing/x/").unwrap_err();
...
fn substitute_e_flag_suppresses_no_match_error() {
    let (editor, _buffer, _window, mut executor) = setup("foo");
    let outcome = executor.execute_line(&editor, r"s/missing/x/e").unwrap();
...
fn substitute_escaped_slash_in_pattern() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("a/b");
    executor.execute_line(&editor, r"s/a\/b/x/").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn substitute_escaped_delimiter_in_replacement() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("foo");
    executor.execute_line(&editor, r"s#foo#f\#o#").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn substitute_ampersand_inserts_whole_match() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("abc");
    executor.execute_line(&editor, r"s/abc/& &/").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn substitute_backreference_reorders_captures() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("ab");
    executor
        .execute_line(&editor, r"s/\(.\)\(.\)/\2\1/")
        .unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn substitute_expression_uses_submatch() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("foo");
    executor
        .execute_line(&editor, r"s/foo/\=submatch(0).'-ok'/")
        .unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn substitute_case_sensitive_by_default() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("Foo\nFoo");
    executor.execute_line(&editor, r"%s/foo/bar/e").unwrap();
    {
        let ed = editor.editor();
        let state = ed.current_buffer().unwrap();
        let text = ed.buffer(state).unwrap().text().unwrap();
...
fn substitute_i_flag_ignores_case() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("Foo\nFoo");
    executor.execute_line(&editor, r"%s/foo/bar/i").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
    assert_eq!(text.to_bytes(), b"bar\nbar");
...
fn substitute_c_flag_errors_without_interactive_ui() {
    ...
    let (editor, _buffer, _window, mut executor) = setup("foo");
    let error = executor.execute_line(&editor, r"s/foo/bar/c").unwrap_err();
...
fn substitute_multiline_newline_pattern_adjusts_extmark_and_undo() {
    ...
    let (editor, buffer, _window, mut executor) = setup("12345\n67890\nxx\n12345\n67890\nxx");
    let (namespace, id) = {
        let mut ed = editor.editor_mut();
        let state = ed.buffer_mut(buffer).unwrap();
    ...
    executor.execute_line(&editor, "1,5s:5\\n:5 ").unwrap();
    {
        let ed = editor.editor();
    ...
    {
        let ed = editor.editor();
        let restored = ed.buffer(buffer).unwrap();
        assert_eq!(
            restored
                .extmarks
                .get(namespace, id)
    ...
    {
        let ed = editor.editor();
        let redone = ed.buffer(buffer).unwrap();
        assert_eq!(
            redone
                .extmarks
                .get(namespace, id)
...
fn ex_delete_default_current_line() {
    ...
    editor
        ...
        .set_window_cursor(window, position(2, 0))
        .unwrap();
    executor.execute_line(&editor, "delete").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
...
fn ex_delete_explicit_range_to_register() {
    let (editor, _buffer, _window, mut executor) = setup("one\ntwo\nthree\nfour");
    executor.execute_line(&editor, "2,3delete a").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
    assert_eq!(text.to_bytes(), b"one\nfour");
    let register = ed.registers().get('a').unwrap().unwrap();
...
fn ex_yank_explicit_range_to_register() {
    let (editor, _buffer, _window, mut executor) = setup("one\ntwo\nthree\nfour");
    executor.execute_line(&editor, "2,3yank a").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
    assert_eq!(text.to_bytes(), b"one\ntwo\nthree\nfour");
...
fn ex_yank_default_current_line_to_unnamed() {
    ...
    editor
        ...
        .set_window_cursor(window, position(2, 0))
        .unwrap();
    executor.execute_line(&editor, "yank").unwrap();
    let ed = editor.editor();
    let register = ed.registers().get('"').unwrap().unwrap();
    assert_eq!(register.to_bytes(), b"two");
}
...
fn ex_put_appends_register_below() {
    ...
    editor
        ...
        .set_window_cursor(window, position(2, 0))
        .unwrap();
    executor.execute_line(&editor, "yank a").unwrap();
    editor
        .editor_mut()
        .set_window_cursor(window, position(1, 0))
        .unwrap();
    executor.execute_line(&editor, "put a").unwrap();
    let ed = editor.editor();
    let state = ed.current_buffer().unwrap();
    let text = ed.buffer(state).unwrap().text().unwrap();
    assert_eq!(text.line(1).unwrap(), b"one");
