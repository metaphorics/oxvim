...
pub fn apply(editor: &mut Editor, buffer: BufHandle, window: WinHandle, operator: Operator, range: EditRange, register: Option<char>, timestamp: i64) -> Result<OperatorResult, OperatorError> {
    ...
    if operator == Operator::Format { return Err(OperatorError::NotImplemented("= reindent requires the formatting subsystem")); }
    ...
    match operator {
        ...
        Operator::Format => return Err(OperatorError::NotImplemented("= reindent requires the formatting subsystem")),
    }
