...
pub fn resolve(
    ...
    match object {
        ...
        'p' => Some(paragraph(lines, cursor, inner, count)),
...
fn offset(starts: &[usize], pos: Position) -> Option<usize> {
    starts
        .get(pos.lnum.saturating_sub(1))
        .copied()
        .map(|start| start.saturating_add(pos.col))
}
fn position(lines: &[Vec<u8>], starts: &[usize], at: usize) -> Position {
    let index = starts.partition_point(|s| *s <= at).saturating_sub(1);
    Position {
        lnum: index + 1,
        col: at
            .saturating_sub(starts[index])
            .min(lines[index].len().saturating_sub(1)),
    }
}
fn range(
    lines: &[Vec<u8>],
    ...
    EditRange {
        start: position(lines, starts, start),
        end: position(lines, starts, end),
        kind,
        inclusive: true,
    }
}
fn class(byte: u8, big: bool) -> u8 {
...
fn word(
    ...
    let (bytes, starts) = flattened(lines);
    let mut at = offset(&starts, cursor)?.min(bytes.len().checked_sub(1)?);
    if class(bytes[at], big) == 0 {
        while at + 1 < bytes.len() && class(bytes[at], big) == 0 {
            at += 1;
        }
    }
    let c = class(bytes[at], big);
    let mut start = at;
    while start > 0 && class(bytes[start - 1], big) == c {
        start -= 1;
    }
    let mut end = at;
    for index in 0..count.max(1) {
        let current = class(bytes[end], big);
        while end + 1 < bytes.len() && class(bytes[end + 1], big) == current {
            end += 1;
        }
        if index + 1 < count.max(1) {
            while end + 1 < bytes.len() && class(bytes[end + 1], big) == 0 {
                end += 1;
            }
            if end + 1 < bytes.len() {
                end += 1;
    ...
    if inner {
        while end > start && class(bytes[end], big) == 0 {
            end -= 1;
        }
    }
...
fn delimited(
    ...
    let (bytes, starts) = flattened(lines);
    let at = offset(&starts, cursor)?;
    let mut left = None;
    let mut depth = 0usize;
    for index in (0..=at.min(bytes.len().saturating_sub(1))).rev() {
    ...
    let mut left = left?;
    let mut right = matching_close(&bytes, left, open, close)?;
    for _ in 1..count.max(1) {
        left = (0..left).rev().find(|index| bytes[*index] == open)?;
        right = matching_close(&bytes, left, open, close)?;
    }
    if inner {
        left += 1;
...
fn quoted(
    ...
    let escaped = |index: usize| {
        ...
        while index > backs && line[index - backs - 1] == b'\\' {
            backs += 1;
        }
        backs % 2 == 1
    };
    let is_quote = |col: usize| col < line.len() && line[col] == quote && !escaped(col);
    let next_quote = |mut col: usize| {
        while col < line.len() {
            if is_quote(col) {
                return col;
            }
    ...
    let (mut left, mut right) = if is_quote(at) {
        ...
        let open = match prev_quote(at) {
            found if found < line.len() => found,
            _ => {
                let first = next_quote(0);
                if first >= line.len() {
                    return None;
                }
                first
    ...
    if !include_quotes {
        ...
        if right + 1 < line.len() && line[right + 1].is_ascii_whitespace() {
            while right + 1 < line.len() && line[right + 1].is_ascii_whitespace() {
                right += 1;
            }
        } else {
            while left > 0 && line[left - 1].is_ascii_whitespace() {
                left -= 1;
            }
        }
    }
...
fn paragraph(lines: &[Vec<u8>], cursor: Position, inner: bool, count: usize) -> EditRange {
    let mut start = cursor.lnum.clamp(1, lines.len());
    while start > 1 && !lines[start - 2].is_empty() {
        start -= 1;
    }
    let mut end = cursor.lnum.clamp(1, lines.len());
    ...
    EditRange {
        start: Position {
            ...
            lnum: end,
            col: lines[end - 1].len().saturating_sub(1),
        },
...
fn sentence(lines: &[Vec<u8>], cursor: Position, inner: bool, count: usize) -> Option<EditRange> {
    let (bytes, starts) = flattened(lines);
    let at = offset(&starts, cursor)?.min(bytes.len().checked_sub(1)?);
    let mut start = at;
    while start > 0 {
        if matches!(bytes[start - 1], b'.' | b'!' | b'?') && terminated(&bytes, start - 1) {
            break;
        }
        start -= 1;
    }
    ...
    for index in 0..count.max(1) {
        while end + 1 < bytes.len()
            && !(matches!(bytes[end], b'.' | b'!' | b'?') && terminated(&bytes, end))
        {
            end += 1;
        }
        if index + 1 < count.max(1) {
    ...
    if !inner {
        while end + 1 < bytes.len() && bytes[end + 1].is_ascii_whitespace() {
            end += 1;
        }
    }
    Some(range(lines, &starts, start, end, MotionKind::CharacterWise))
