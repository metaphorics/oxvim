...
// Test module: unwrap/expect/panic on compile results IS the assertion, and
// `_spec` bindings are side-effect-free upstream-citation anchors per case.
#![allow(
    clippy::unwrap_used,
    clippy::expect_used,
    clippy::panic,
...
use ox_regex::{Engine, ExecError, Magic, Match, Position, Text, compile, exec, exec_at, try_exec};

fn matched(pattern: &str, magic: Magic, text: &Text) -> Option<String> {
    let prog =
        compile(pattern, magic).unwrap_or_else(|error| panic!("compile {pattern:?}: {error}"));
...
macro_rules! case {
    ($name:ident, $citation:literal, $pattern:literal, $magic:expr, $text:literal, $expected:expr) => {
        ...
        fn $name() {
            let _spec = $citation;
            assert_eq!(
                matched($pattern, $magic, &Text::from($text)),
...
case!(
    empty_pattern_matches,
    "regexp.c:vim_regexec empty-match behavior",
    "",
    Magic::Magic,
    "abc",
    Some("")
...
case!(
    very_magic_lookahead,
    "pattern.txt:424-443,676-690",
    "\\v(foo)@=foo",
    Magic::Magic,
    "foo",
    Some("foo")
...
case!(
    very_magic_noncapturing,
    "pattern.txt:424-443,371-380",
    "\\v%(foo|bar)",
    Magic::Magic,
    "bar",
    Some("bar")
...
case!(
    very_magic_optional_sequence,
    "pattern.txt:424-443,1229-1248",
    "\\vr%[ead]",
    Magic::Magic,
    "read",
    Some("read")
...
case!(
    very_magic_position_assertion,
    "pattern.txt:424-443,955-980",
    "\\v%2cb",
    Magic::Magic,
    "ab",
    Some("b")
...
case!(
    magic_switch_changes_group_close,
    "pattern.txt:420-443",
    "\\v(foo\\M\\)",
    Magic::Magic,
    "foo",
...
case!(
    lookbehind_stops_after_previous_line,
    "pattern.txt:734-738",
    "\\(foo\\nbar\\n\\)\\@<=baz",
    Magic::Magic,
    "foo\nbar\nbaz",
...
case!(
    limited_lookbehind_cross_line_long_prefix,
    "pattern.txt:752-758",
    "\\(\\_.\\{6}\\)\\@5<=span",
    Magic::Magic,
    "abcd\nwxyzspan",
...
case!(
    atomic_group_prevents_backtrack,
    "pattern.txt:783-792",
    "\\(a*\\)\\@>a",
    Magic::Magic,
    "aaa",
...
case!(
    virtual_column_anchor,
    "pattern.txt:981-1014",
    "\\%2vb",
    Magic::Magic,
    "abc",
    Some("b")
...
case!(
    keyword_matches_non_ascii_default_iskeyword,
    "test_functions.vim:Test_matchstrlist",
    "\\k\\+",
    Magic::Magic,
    "😊😊",
    Some("😊😊")
...
case!(
    ordinary_negated_collection_stops_at_line,
    "pattern.txt:1103-1108",
    "a[^x]b",
    Magic::Magic,
    "a\nb",
...
case!(
    captured_backreference,
    "test_regexp_latin.vim:75-81",
    "\\(e\\)\\1",
    Magic::Magic,
    "three",
    Some("ee")
...
case!(
    unicode_ignore_case_range,
    "pattern.txt:1103-1160,1260-1282",
    "\\c[À-Ö]",
    Magic::Magic,
    "à",
    Some("à")
...
case!(
    nested_capture_oracle,
    "test_regexp_latin.vim:54-59",
    "\\(\\(a[a-d] \\)*\\)\\(x\\)",
    Magic::Magic,
    "aa ab x",
    Some("aa ab x")
...
case!(
    optional_capture_oracle,
    "test_regexp_latin.vim:66-70",
    "\\(abc\\>\\)\\?\\s*\\(def\\)",
    Magic::Magic,
    "abc def",
    Some("abc def")
...
fn captures_report_byte_positions() {
    ...
    assert_eq!(
        (
            found.captures[0].as_ref().unwrap().start.byte,
            found.captures[0].as_ref().unwrap().end.byte
        ),
        (3, 5)
    );
}
...
fn automatic_engine_routes_backrefs_to_bounded_bt() {
    ...
    assert_eq!(
        compile("\\(a\\)\\1", Magic::Magic).unwrap().engine(),
        Engine::Backtracking
    );
}
...
fn automatic_engine_routes_lookbehind_to_bounded_bt() {
    ...
    assert_eq!(
        compile("\\(a\\)\\@<=b", Magic::Magic).unwrap().engine(),
        Engine::Backtracking
    );
}
...
fn engine_prefix_forces_backtracking() {
    ...
    assert_eq!(
        compile("\\%#=1abc", Magic::Magic).unwrap().engine(),
        Engine::Backtracking
    );
}
...
fn automatic_engine_routes_pathological_range_to_bt() {
    ...
    assert_eq!(
        compile("a\\{0,501}", Magic::Magic).unwrap().engine(),
        Engine::Backtracking
    );
...
fn explicit_recursion_limit_is_typed() {
    ...
    // Depth counts syntactic nesting, not repetition iterations: a flat
    // `a*` run of any length stays under the limit, while nested groups
    // trip it.
    let prog = compile("\\%#=1a*", Magic::Magic)
        .unwrap()
        .with_limits(1_000, 2);
    assert_eq!(
        try_exec(&prog, &Text::from("aaaa")),
    ...
    let nested = compile("\\%#=1\\(\\(\\(a\\)\\)\\)", Magic::Magic)
        .unwrap()
        .with_limits(1_000, 2);
    assert_eq!(
        try_exec(&nested, &Text::from("a")),
        Err(ExecError::RecursionLimit)
    );
}
