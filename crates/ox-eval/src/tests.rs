    ...
    fn call(&mut self, name: &OxStr, mut args: Vec<Typval>, _scope: &mut Scope) -> Result<Typval> {
        match name.as_bytes() {
            b"id" => args
                .into_iter()
                .next()
                .ok_or_else(|| EvalError::new("E119", 0, "argument required")),
            b"len" => match args.first() {
                Some(Typval::List(v)) => Ok(Typval::Number(
                    i64::try_from(v.borrow().items.len()).unwrap_or(i64::MAX),
                )),
                Some(Typval::String(v)) => Ok(Typval::Number(
                    i64::try_from(v.as_bytes().len()).unwrap_or(i64::MAX),
                )),
                Some(Typval::Dict(v)) => Ok(Typval::Number(
                    i64::try_from(v.borrow().entries.len()).unwrap_or(i64::MAX),
                )),
            ...
            b"append" => {
                if args.len() != 2 {
                    return Err(EvalError::new("E119", 0, "two arguments required"));
                }
                let rhs = args.pop().unwrap();
                let lhs = args.pop().unwrap();
                match lhs {
                    Typval::List(list) => {
                        list.borrow_mut().items.push(rhs);
                        Ok(Typval::List(list))
                    }
...
eval_cases!(
    ...
    (
        precedence_subtract_left,
        b"10 - 4 - 2",
        Typval::Number(4),
        "eval.c:2424"
    ),
    (
        precedence_divide_left,
        b"20 / 4 / 2",
        Typval::Number(2),
        "eval.c:2580"
    ),
    (
        precedence_mixed_arithmetic,
        b"10 - 2 * 3 + 4",
        Typval::Number(8),
        "vimeval.txt:856-867"
    ),
    ...
    (
        precedence_and_or_left,
        b"0 && 1 || 1",
        Typval::Number(1),
        "eval.c:2013-2134"
    ),
    (
        precedence_ternary_low,
        b"1 ? 2 + 3 : 4 * 5",
        Typval::Number(5),
        "vimeval.txt:907-923"
    ),
    ...
    (
        ...
        b"+--5",
        Typval::Number(5),
        "vimeval.txt:1152-1167"
    ),
    (
        literal_decimal,
        b"12345",
        Typval::Number(12345),
        "vimeval.txt:1331-1353"
    ),
    (
        literal_hex,
        b"0x1f",
        Typval::Number(31),
        "vimeval.txt:1333-1343"
    ),
    (
        literal_legacy_octal,
        b"010",
        Typval::Number(8),
        "vimeval.txt:1333-1343"
    ),
    (
        literal_explicit_octal,
        b"0o77",
        Typval::Number(63),
        "vimeval.txt:1333-1343"
    ),
    ...
    (
        literal_double_escapes,
        b"\"a\\nb\\tc\\\"\\\\\"",
        Typval::String(OxStr(b"a\nb\tc\"\\".to_vec())),
        "vimeval.txt:1413-1448"
    ),
    (
        literal_single_quote,
        b"'a''b'",
        Typval::String(OxStr(b"a'b".to_vec())),
        "vimeval.txt:1461-1474"
    ),
    (
        literal_blob,
        b"0z0102.0304",
        Typval::Blob(vec![1, 2, 3, 4]),
        "vimeval.txt:1451-1458"
    ),
    (
        literal_nested_list,
        b"[1, 'two', [3]]",
    ...
    (
        literal_list_trailing_comma,
        b"[1, 2,]",
        Typval::list(vec![Typval::Number(1), Typval::Number(2)]),
        "eval.c:3902"
    ),
    (
        literal_dict,
        b"{'a': 1, 'b': 2,}",
        Typval::dict(vec![
            (OxStr::from("a"), Typval::Number(1)),
            (OxStr::from("b"), Typval::Number(2))
        ]),
        "vimeval.txt:883"
    ),
    (
    ...
    (
        coerce_not_number_text,
        b"!'123'",
        Typval::Number(0),
        "typval.c:4306-4311"
    ),
    (
        coerce_not_zero_text,
        b"!'0'",
        Typval::Number(1),
        "typval.c:4306-4311"
    ),
    (
        coerce_numeric_prefix,
        b"1 + '20abc'",
        Typval::Number(21),
        "eval.c:2351-2409"
    ),
    ...
    (
        coerce_two_strings_add,
        b"'10' + '20'",
        Typval::Number(30),
        "vimeval.txt:1114-1120"
    ),
    (
        concat_numbers_dot,
        b"1 . 2",
        Typval::String(OxStr::from("12")),
        "vimeval.txt:1117-1120"
    ),
    (
        concat_numbers_dotdot,
        b"1 .. 2",
        Typval::String(OxStr::from("12")),
        "vimeval.txt:1097-1107"
    ),
    (
        float_promotion,
        b"1.5 + 2",
    ...
    (
        coalesce_zero_string_truthy,
        b"'0' ?? 456",
        Typval::String(OxStr::from("0")),
        "typval.c:4778-4804"
    ),
    (
        coalesce_empty_list,
    ...
    (
        ...
        b"'abc' == 'abc'",
        Typval::Number(1),
        "vimeval.txt:993-1017"
    ),
    (
        compare_case_sensitive,
        b"'abc' ==# 'ABC'",
        Typval::Number(0),
        "vimeval.txt:1075-1081"
    ),
    (
        compare_case_insensitive,
        b"'abc' ==? 'ABC'",
        Typval::Number(1),
        "vimeval.txt:1078-1081"
    ),
    (
        compare_string_number,
        b"'0' == 0",
        Typval::Number(1),
        "vimeval.txt:1062-1069"
    ),
    (
        compare_non_numeric_string_number,
        b"'abc' == 0",
        Typval::Number(1),
        "vimeval.txt:1062-1069"
    ),
    (
        compare_strings_no_numeric_coercion,
        b"'0' == '00'",
        Typval::Number(0),
        "vimeval.txt:1071-1077"
    ),
    (
        compare_float_number,
        b"1.0 == 1",
        Typval::Number(1),
        "eval.c:6815"
    ),
    (
        ...
        b"[1, 2] == [1, 2]",
        Typval::Number(1),
        "vimeval.txt:1025-1029"
    ),
    (
        compare_list_not_equal,
        b"[1, 2] != [1, 3]",
        Typval::Number(1),
        "vimeval.txt:1025-1029"
    ),
    ...
    (
        identity_distinct_lists,
        b"[1] is [1]",
        Typval::Number(0),
        "vimeval.txt:1049-1053"
    ),
    (
        identity_distinct_lists_not,
        b"[1] isnot [1]",
        Typval::Number(1),
        "vimeval.txt:1049-1053"
    ),
    (
        ...
        "vimeval.txt:1182-1202"
    ),
    (
        index_string_last,
        b"'axb'[2]",
        Typval::String(OxStr::from("b")),
        "vimeval.txt:1193-1202"
    ),
    (
        index_string_oob,
        b"'axb'[3]",
        Typval::String(OxStr::from("")),
        "vimeval.txt:1200-1202"
    ),
    (
    ...
    (
        slice_string_prefix,
        b"'editor'[:3]",
        Typval::String(OxStr::from("edit")),
        "vimeval.txt:1214-1228"
    ),
    (
        slice_string_middle,
        b"'editor'[2:4]",
        Typval::String(OxStr::from("ito")),
        "vimeval.txt:1214-1228"
    ),
    (
        slice_string_negative,
        b"'editor'[-3:]",
        Typval::String(OxStr::from("tor")),
        "vimeval.txt:1230-1241"
    ),
    (
        index_list_first,
        b"[10, 20, 30][0]",
        Typval::Number(10),
        "vimeval.txt:1204-1211"
    ),
    (
        index_list_negative,
        b"[10, 20, 30][-1]",
    ...
    (
        slice_list_middle,
        b"[1, 2, 3, 4][1:2]",
        Typval::list(vec![Typval::Number(2), Typval::Number(3)]),
        "vimeval.txt:1243-1249"
    ),
    (
        slice_list_prefix,
        b"[1, 2, 3][:1]",
    ...
    (
        ...
        b"[1, 2][0:8]",
        Typval::list(vec![Typval::Number(1), Typval::Number(2)]),
        "test_listdict.vim:37"
    ),
    (
        slice_list_empty_high,
        b"[1, 2][8:]",
        Typval::list(vec![]),
        "test_listdict.vim:38"
    ),
    (
        slice_list_empty_low,
        b"[1, 2][-3:-1]",
        Typval::list(vec![]),
        "test_listdict.vim:47"
    ),
    (
        dict_index_string,
        b"{'name': 'ox'}['name']",
        Typval::String(OxStr::from("ox")),
        "vimeval.txt:1266-1280"
    ),
    (
        dict_member,
        b"{'name': 'ox'}.name",
    ...
    (
        dict_index_number_key,
        b"{0: 'zero'}[0]",
        Typval::String(OxStr::from("zero")),
        "test_expr.vim:111"
    ),
    (
        dict_nested_members,
    ...
    (
        dict_member_then_index,
        b"{'a': [10, 20]}.a[1]",
        Typval::Number(20),
        "vimeval.txt:1173-1179"
    ),
    (
        lambda_nullary,
        b"{-> 42}()",
        Typval::Number(42),
        "vimeval.txt:1608-1626"
    ),
    (
        lambda_binary,
        b"{a, b -> a + b}(10, 20)",
    ...
    (
        ...
        b"{x -> x * 2}(21)",
        Typval::Number(42),
        "vimeval.txt:1611-1626"
    ),
    (
        lambda_returns_list,
        b"{x -> [x, x + 1]}(3)",
        Typval::list(vec![Typval::Number(3), Typval::Number(4)]),
        "vimeval.txt:1641-1647"
    ),
    (
        lambda_variadic_count,
        b"{... -> a:0}(1, 2, 3)",
        Typval::Number(3),
        "test_lambda.vim:55-58"
    ),
    (
        lambda_variadic_list,
        b"{... -> a:000}(7, 8)",
        Typval::list(vec![Typval::Number(7), Typval::Number(8)]),
        "userfunc.txt:a:000"
    ),
    (
        lambda_variadic_index,
        b"{... -> a:1}(10, 20)",
        Typval::Number(10),
        "userfunc.txt:a:1"
    ),
    (
        lambda_named_then_variadic,
        b"{a, ... -> a + a:0}(1, 2, 3)",
        Typval::Number(3),
        "test_lambda.vim:55-62"
    ),
    (
        lambda_extra_args_accepted,
        b"{a -> a}(1, 2, 3)",
        Typval::Number(1),
        "userfunc.c:396 uf_varargs=true"
    ),
    (
        string_multibyte_fold_equal,
        b"\"\xc3\x84\" ==? \"\xc3\xa4\"",
        Typval::Number(1),
        "mbyte.c:mb_stricmp"
    ),
    (
        string_multibyte_fold_sensitive,
        b"\"\xc3\x84\" ==# \"\xc3\xa4\"",
        Typval::Number(0),
        "vimeval.txt:1075-1081"
    ),
    (
        turkish_dotted_i_fold_equal,
        b"\"\xc4\xb0\" ==? \"i\xcc\x87\"",
        Typval::Number(1),
        "mbyte.c:mb_stricmp"
    ),
    (
        turkish_dotted_i_fold_equal_reverse,
        b"\"i\xcc\x87\" ==? \"\xc4\xb0\"",
        Typval::Number(1),
        "mbyte.c:mb_stricmp"
    ),
    (
        turkish_dotted_i_fold_sensitive,
        b"\"\xc4\xb0\" ==# \"i\xcc\x87\"",
        Typval::Number(0),
        "vimeval.txt:1075-1081"
    ),
    (
        casefold_ordering_lower_first,
        b"'a' <? 'B'",
        Typval::Number(1),
        "mbyte.c:mb_stricmp"
    ),
    (
        casefold_ordering_upper_greater,
        b"'B' >? 'a'",
        Typval::Number(1),
        "mbyte.c:mb_stricmp"
    ),
    (
        turkish_dotted_i_fold_ordering,
        b"\"\xc4\xb0\" >? \"I\"",
        Typval::Number(1),
        "mbyte.c:mb_stricmp"
    ),
    (
        ...
        b"[1, 2, 3]->len()",
        Typval::Number(3),
        "vimeval.txt:1292-1305"
    ),
    (
        method_chain,
        b"[1, 2, 3]->len()->id()",
        Typval::Number(3),
        "vimeval.txt:1300-1305"
    ),
    (
        method_receiver_first,
        b"10->sum(20, 30)",
        Typval::Number(60),
        "vimeval.txt:1295-1298"
    ),
    (
        ctrl_d_escape_is_control_byte,
        b"\"\\<C-D>\"",
        Typval::String(OxStr(vec![0x04])),
        "charset.c:trans_special"
    ),
    (
        ctrl_w_escape_is_control_byte,
        b"\"\\<C-W>\"",
        Typval::String(OxStr(vec![0x17])),
        "charset.c:trans_special"
    )
);
...
error_cases!(
    ...
    (
        error_duplicate_dict_key,
        b"{'a': 1, 'a': 2}",
        "E721",
        "eval.c:E721"
    ),
    (
        error_duplicate_lambda_param,
        b"{a, a -> a + a}(1, 2)",
        "E853",
        "test_lambda.vim:61"
    ),
    (
        error_duplicate_lambda_param_types,
        b"{list, list -> 1}([1], [2])",
        "E853",
        "userfunc.c:134"
    ),
    (
        error_list_relational_compare,
        b"[1] > [2]",
        "E692",
        "eval.c:6789-6791"
    ),
    (
        error_dict_relational_compare,
        b"{'a': 1} > {'a': 0}",
        "E736",
        "eval.c:6822-6828"
    ),
    ...
    (
        error_dictionary_slice,
        b"{'a': 1}[0:0]",
        "E719",
        "eval.c:eval_index_inner E719"
    )
);
...
fn float_concatenates_as_a_string_and_only_indexing_is_e806() {
    ...
    assert_eq!(
        value(b"1 . 90 * 90.0"),
        Typval::String(OxStr::from("18100.0"))
    );
    assert_eq!(value(b"(1.0/0.0) . ''"), Typval::String(OxStr::from("inf")));
    assert_eq!(
        value(b"(-1.0/0.0) . ''"),
        Typval::String(OxStr::from("-inf"))
    );
...
fn same_variable_preserves_list_identity() {
    ...
    scope
        .set(b"xs", Typval::list(vec![Typval::Number(1)]))
        .unwrap();
    assert_eq!(value_in(b"xs is xs", &mut scope), Typval::Number(1));
}

#[test]
fn lambda_resolves_across_separate_evaluators() {
    // A lambda is a value: calling it through a different evaluator — one
    // whose closure registry differs, like a `map()` callback dispatched by
    // the builtin host — must reach the original closure, not E117.
    let lambda = {
    ...
    assert!(matches!(lambda, Typval::Partial(_)));

    let mut host = Host;
    let regex = Regex;
    let mut scope = Scope::new();
    let mut caller = Evaluator::new(&mut host, &regex);
    let applied = caller
        .invoke(lambda.clone(), vec![Typval::Number(41)], &mut scope)
        .unwrap();
    assert_eq!(applied, Typval::Number(42));

    // Too few arguments is an arity error, not unknown-function.
    let arity = caller.invoke(lambda, Vec::new(), &mut scope).unwrap_err();
    assert_eq!(arity.code, "E119");
}

#[test]
fn lambda_with_unknown_registry_stays_unknown() {
    // The fallback resolves live registries only; a nonce that never
    // existed keeps the honest E117 instead of inventing a result.
    let mut host = Host;
    let regex = Regex;
    let mut scope = Scope::new();
    let phantom = Typval::Partial(Funcref {
        name: OxStr::from("<lambda>404"),
        args: Vec::new(),
        dict: None,
        registry: Some(usize::MAX),
    });
    let unknown = Evaluator::new(&mut host, &regex)
        .invoke(phantom, vec![Typval::Number(1)], &mut scope)
        .unwrap_err();
    assert_eq!(unknown.code, "E117");
    assert_eq!(unknown.message, "Unknown function: <lambda>404");
}
...
fn repeated_list_index_preserves_nested_identity() {
    ...
    scope
        .set(
            b"xs",
            Typval::list(vec![Typval::list(vec![Typval::Number(1)])]),
        )
        .unwrap();
...
fn repeated_dict_member_preserves_nested_identity() {
    ...
    scope
        .set(
            b"d",
            Typval::dict(vec![(OxStr::from("x"), Typval::list(vec![]))]),
        )
        .unwrap();
...
fn bare_global_scope_returns_dictionary() {
    ...
    scope.global.push((OxStr::from("x"), Typval::Number(7)));
    assert_eq!(
        value_in(b"g:", &mut scope),
        Typval::dict(vec![(OxStr::from("x"), Typval::Number(7))])
    );
}
...
fn vim_scope_is_read_only() {
    ...
    let error = Scope::new()
        .set_scoped(ScopeKind::Vim, b"x", 4, Typval::Number(1))
        .unwrap_err();
...
#[test]
#[cfg(unix)]
fn environment_reads_live_process_state() {
    use std::os::unix::ffi::OsStrExt;
    // runtime/doc/vimeval.txt:889; `$VAR` reads the live process environment
    // byte-for-byte -- including values that are not valid UTF-8 -- and sees
    // mutations made after the Scope was constructed.
    const NAME: &str = "OXVIM_TEST_EVAL_LIVE_ENV";
    let _guard = EnvGuard::take(NAME);
    let read = format!("${NAME}");
    let mut scope = Scope::new();
    assert_eq!(
        value_in(read.as_bytes(), &mut scope),
        Typval::String(OxStr(Vec::new()))
    );

    assert!(ox_sys::set_env(
        NAME,
        std::ffi::OsStr::from_bytes(b"\xff\xfe")
    ));
    assert_eq!(
        value_in(read.as_bytes(), &mut scope),
        Typval::String(OxStr(vec![0xff, 0xfe]))
    );
}
...
fn register_lookup() {
    ...
    assert_eq!(
        value_in(b"@a", &mut scope),
        Typval::String(OxStr::from("text"))
    );
}
...
fn closure_captures_snapshot() {
    ...
    assert_eq!(
        evaluator.eval(&call, &mut call_scope).unwrap(),
        Typval::Number(10)
    );
}
...
fn nested_parentheses(levels: usize) -> Vec<u8> {
    let mut source = vec![b'('; levels];
    source.push(b'1');
    source.extend(std::iter::repeat_n(b')', levels));
    source
}

#[test]
fn parser_accepts_expression_just_below_default_recursion_limit() {
    let source = nested_parentheses(crate::parser::DEFAULT_MAX_NESTING - 1);
    assert_eq!(value(&source), Typval::Number(1));
}

#[test]
fn parser_default_recursion_limit_returns_typed_error() {
    let source = nested_parentheses(crate::parser::DEFAULT_MAX_NESTING);
    let error = Parser::new(&source).parse().unwrap_err();
    assert_eq!(error.code, "E1169");
    assert_eq!(error.offset, crate::parser::DEFAULT_MAX_NESTING);
}

#[test]
fn parser_recursion_limit_is_typed() {
    // brief section 2; E1169 recursion guard.
    let error = Parser::new(b"(((1)))")
        .with_max_nesting(2)
        .parse()
        .unwrap_err();
    assert_eq!(error.code, "E1169");
...
fn evaluator_condition_uses_numeric_coercion() {
    ...
    assert!(
        !Evaluator::<Host, Regex>::condition_number(&Typval::String(OxStr::from("0"))).unwrap()
    );
}
...
fn variadic_lambda_partial_binds_leading_args() {
    ...
    let Typval::Partial(funcref) = closure else {
...
fn lambda_name_is_upstream_compatible() {
    ...
    let Typval::Partial(funcref) = value else {
        panic!("expected a Partial")
    };
...
#[test]
fn closure_from_isolated_evaluator_resolves_to_its_own_closure() {
    // Two independent registries each create a <lambda>0. A Partial from the
    // first registry still reaches its own closure — and never the second
    // registry's same-index closure: resolution goes by registry nonce, like
    // upstream's refcounted func_T that stays callable from anywhere.
    let mut first_host = Host;
    let mut second_host = Host;
    let regex = Regex;
    let mut creator = Evaluator::new(&mut first_host, &regex);
    ...
    // The caller's registry also holds a closure at index 0.
    ...
    // 6 = x + 1 from closure_a; the second registry's x * 2 would give 10.
    assert_eq!(caller.eval(&call, &mut store).unwrap(), Typval::Number(6));
}
...
fn interpolated_strings_evaluate_literals_expressions_and_escapes() {
    ...
    assert_eq!(
        value(br#"$"foo{1 + 2}bar""#),
        Typval::String(OxStr::from("foo3bar"))
    );
    assert_eq!(
        value(br#"$"{{x}}={v:true}""#),
        Typval::String(OxStr::from("{x}=v:true"))
    );
    assert_eq!(
        value(br"$'left{'mid'}right'"),
        Typval::String(OxStr::from("leftmidright"))
    );
    assert_eq!(
        value(br#"$"outer{$"{'inner'}"}end""#),
        Typval::String(OxStr::from("outerinnerend"))
    );
}
...
fn literal_dictionary_missing_colon_stays_e720() {
    ...
    assert_eq!(
        error(b"#{a.b: 1}").message,
        "Missing colon in Dictionary: .b: 1}"
    );
}
...
fn method_call_white_space_after_the_arrow_is_e15() {
    ...
    assert_eq!(
        error(b"[1, 2, 3]-> len() + 1").message,
        "Invalid expression: \" len() + 1\""
    );
...
fn method_call_without_parentheses_is_e107() {
    ...
    assert_eq!(
        error(b"'t'->{x -> x}").message,
        "Missing parentheses: lambda"
    );
