...
/// A trailing empty line with `has_eol = false` must survive: the joined
/// text ends in the separator, so re-deriving `has_eol` from it would
/// silently drop the last line (`Buffer::from_lines` honor contract).
#[test]
fn from_lines_keeps_trailing_empty_line_without_eol() {
    let noeol = Buffer::from_lines(&bytes(&["a", "", ""]), false).unwrap();
    assert_eq!(noeol.line_count(), 3);
    assert!(!noeol.has_eol());
    assert_eq!(noeol.to_bytes(), b"a\n\n");

    let eol = Buffer::from_lines(&bytes(&["a", "", ""]), true).unwrap();
    assert_eq!(eol.line_count(), 3);
    assert!(eol.has_eol());
    assert_eq!(eol.to_bytes(), b"a\n\n\n");
}
...
fn undo_file_read_rejects_trailing_and_truncated_tails() {
    ...
    tree.record(edit_at(2, &bytes(&["b"]), &bytes(&["c"])), 1_710_000_001);
...
fn unsynced_edits_join_one_undo_block() {
    ...
    assert_eq!(
        tree.summary().seq_last,
        first,
        "no extra header was created"
    );
...
fn undojoin_reopens_the_newest_block_but_never_after_an_undo() {
    ...
    assert_eq!(
        joined, first,
        "undojoin put the edit in the existing header"
    );
    ...
    assert_ne!(
        tree.current_seq(),
        0,
        "stopped on a header, not at the root"
    );
    assert_eq!(tree.undojoin(), Err(UndoError::JoinAfterUndo));
}
...
fn shada_round_trip_size_limit_and_merge() {
    let old = ShaDaEntry::new(
        ...
        10,
        Value::Map(vec![
            (Value::from("n"), Value::from(u64::from(b'a'))),
            (
                Value::from("rc"),
                Value::Array(vec![Value::Binary(b"old".to_vec())]),
            ),
        ]),
    );
    let new = ShaDaEntry {
        ...
        data: Value::Map(vec![
            ...
            (
                Value::from("rc"),
                Value::Array(vec![Value::Binary(b"new".to_vec())]),
            ),
        ]),
        ..old.clone()
    };
    let merged = ShaDa { entries: vec![old] }.merge(&ShaDa {
    ...
    let stream = ShaDa {
        entries: vec![
            ShaDaEntry::new(ShaDaEntryType::Header, 1, Value::Map(vec![])),
            ShaDaEntry::new(
                ShaDaEntryType::SubString,
                2,
                Value::Array(vec![Value::Binary(vec![b'x'; 2048])]),
            ),
        ],
    };
...
fn forward_undo_dumps(dir: &Path, text: &[u8], undo_bytes: &[u8]) -> Vec<PathBuf> {
    ...
    let mut args = vec![
        ...
        "-c".into(),
        command_arg(format!("rundo {}", undo_path.display())),
    ];
    ...
    for step in 0..4 {
        ...
        args.push("-c".into());
        args.push(command_arg(format!(
            "silent! call writefile(getline(1, '$'), '{}')",
            out.display()
        )));
    }
    args.push("-c".into());
...
fn ox_text_writes_undo_history_real_nvim_undoes_forward() {
    ...
    tree.sync();
    tree.record(edit_at(2, &bytes(&["c"]), &bytes(&["d"])), 1_710_000_002);
    buffer.replace_lines(2, 2, &bytes(&["d"])).unwrap();
    tree.sync();
    tree.record(edit_at(3, &[], &bytes(&["e"])), 1_710_000_003);
...
fn ox_text_writes_branching_history_real_nvim_follows_active_branch() {
    ...
    tree.undo().unwrap();
    tree.record(edit_at(2, &bytes(&["c"]), &bytes(&["e"])), 1_710_000_003);
...
fn ox_text_writes_a_grouped_block_real_nvim_undoes_it_in_one_step() {
    ...
    for gone in ["a", "b"] {
        tree.record(edit_at(1, &bytes(&[gone]), &[]), 1_710_000_001);
    ...
    undo.write(&mut undo_bytes).unwrap();
    assert_eq!(
        written_header_count(&undo_bytes),
        1,
        "two edits, one header"
    );
...
// End-to-end real-nvim interop scenario; splitting would obscure the flow.
...
fn real_nvim_undo_and_shada_interoperate_both_directions() {
    ...
    run_nvim(&[
        ...
        "-c".into(),
        command_arg("normal GoTHREE"),
        "-c".into(),
        command_arg("write"),
        "-c".into(),
        command_arg(format!("wundo! {}", upstream_undo.display())),
        "-c".into(),
        "qa!".into(),
    ]);
    ...
    run_nvim(&[
        ...
        text.display().to_string(),
        "-c".into(),
        command_arg(format!("rundo {}", emitted_undo.display())),
        "-c".into(),
        "undo".into(),
        "-c".into(),
        command_arg(format!(
    ...
    run_nvim(&[
        ...
        "NONE".into(),
        "-i".into(),
        upstream_shada.display().to_string(),
        "-c".into(),
        "let @a='oracle'".into(),
        "-c".into(),
        "wshada!".into(),
    ...
    run_nvim(&[
        ...
        "NONE".into(),
        "-i".into(),
        emitted_shada.display().to_string(),
        "-c".into(),
        command_arg(format!(
            "call writefile([getreg('a')], '{}')",
            register_result.display()
        )),
        "-c".into(),
        "qa!".into(),
    ]);
        ...
        entries: vec![
            ...
            ShaDaEntry::new(
                ...
                Value::Map(vec![
                    (
                        Value::from("rc"),
                        Value::Array(vec![Value::Binary(b"from-ox".to_vec())]),
                    ),
                    (Value::from("n"), Value::from(u64::from(b'a'))),
                ]),
            ),
...
fn real_nvim_swap_interoperates_both_directions() {
    ...
    assert_eq!(
        fs::read_to_string(recovered).unwrap(),
        "red\ngreen\nblue\nORANGE\n"
    );
    fs::remove_dir_all(dir).unwrap();
}
...
fn undo_file_header_seq_validation() {
    ...
    assert_eq!(
        ...
        2,
        "fixture must contain two headers"
    );
