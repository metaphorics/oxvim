...
//!
//! ```text
//! cargo run -p api-inventory-gen > crates/ox-api/src/api_function_names.rs
//! ```
//!
...
fn generate(functions: &[Function]) -> Result<String, GenError> {
    ...
    out.push_str(
        "// Generated by `cargo run -p api-inventory-gen` from the canonical msgpack\n\
...
fn run(check: bool) -> Result<(), GenError> {
    ...
    if check {
        ...
        if existing != output {
            eprintln!(
                "api-inventory-gen: checked-in file does not match generated output.\n\
                 Run `cargo run -p api-inventory-gen > crates/ox-api/src/api_function_names.rs`."
            );
            return Err(GenError::BadShape("drift detected"));
        }
        eprintln!(
            "api-inventory-gen: checked-in file is up to date ({} functions).",
            functions.len()
        );
        return Ok(());
    }
...
fn main() -> ExitCode {
    ...
    match run(check) {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            eprintln!("api-inventory-gen: {err}");
            ExitCode::FAILURE
        }
    }
}
    ...
    fn checked_in_inventory_matches_canonical_regeneration() {
        ...
        assert!(
            ...
            "checked-in api_function_names.rs is stale; regenerate with \
             cargo run -p api-inventory-gen: {result:?}"
        );
    }
}
