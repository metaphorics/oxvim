...
fn verification_three_embedded_scenario() {
    ...
    let info = result(
        &editor
            .request(1, "nvim_get_api_info", vec![])
            .expect("get api info")
            .0,
    );
    ...
    assert_eq!(
        map_get(
            map_get(fields.get(1).expect("metadata"), "version"),
            "api_level"
        )
        .as_i64(),
        Some(15)
    ...
    assert_eq!(
        result(
            &editor
                .request(5, "nvim_command", vec![Value::from("normal! ggdd")])
                .expect("normal delete")
                .0
        ),
        Value::Nil
    );
