...
use differential::{
    ORACLE, OXVIM, binary, divergence_fingerprint, load_session, read_skips, readable_diff,
    run_session,
};
...
fn run() -> Result<(), String> {
    ...
    if bless {
        let value = reason
            .as_deref()
            .ok_or_else(|| "--bless requires a non-empty --reason justification".to_owned())?;
    ...
    for path in selected {
        ...
        if bless {
            ...
            skips.retain(|existing| {
                !existing.starts_with(&session_prefix) && !existing.starts_with(&legacy_prefix)
            });
            skips.push(line);
            ...
            .iter()
            .any(|line| line.starts_with(&key) && line.len() > key.len())
        {
    ...
    if failed {
        Err("unsanctioned session divergences found".to_owned())
    } else {
        Ok(())
    }
}
